...
**Domain:** Docker-based video processing pipeline (Whisper transcription + Remotion rendering)
...
pip install uvicorn[standard]==0.46.0
pip install python-multipart==0.0.27
pip install pydantic==2.13.3

# === Node.js container (Remotion + Render) ===
npm install remotion@4.0.457 @remotion/cli@4.0.457 @remotion/renderer@4.0.457
npm install @remotion/bundler@4.0.457 @remotion/captions@4.0.457
npm install @remotion/media-utils@4.0.457 @remotion/paths@4.0.457
npm install bullmq@5.76.5 ioredis@5.12.1
npm install express@5.2.1 zod@4.4.3 uuid@14.0.0
...
**If processing Spanish-language content (likely given project context):**
...
**Key pattern:** Each container reads from and writes to the shared volume (`/data/{pipeline-id}/`). This makes every step's output inspectable — you can check `transcript.json` or `silences.json` before proceeding to the next step.
...
**Recommendation:** Start with `medium` for development (works on CPU and GPU). Upgrade to `turbo` or `large-v3` in production with GPU.
...
---
*Stack research for: Docker-based video processing pipeline (Whisper + Remotion)*
*Researched: 2026-05-05*