...
- [x] 20-04-PLAN.md — Renderer sync (cp TitleOverlay + pipeline-config to renderer), full vitest suite, build verify, human visual check (TITLE-01, TITLE-02, TITLE-03)
...
- [x] 21-02-PLAN.md — Render path: PngOverlay.tsx composition, SubtitledVideo integration, render.ts base64 decode + upscale warning, Root.tsx defaultProps, renderer sync (OVERLAY-01, OVERLAY-02, OVERLAY-03)
...
- [x] 22-06-PLAN.md — Human visual sign-off on port 3123 against all 5 ROADMAP success criteria (D-01..D-11)
...
- [x] 23-03-PLAN.md — Font resilience: @remotion/fonts@4.0.457 local-first → gstatic-retry → bundled-sans chain + per-font timeout in both fonts.ts, vendored woff2, Dockerfile COPY public/ (RENDER-05)

**Wave 2** *(blocked on 23-01)*

- [x] 23-02-PLAN.md — Studio proxy routes: POST /api/render → api-server POST /batch (queue, jobId up front), GET /api/status/:id, GET /api/result/:id (Range-aware) + server.test.ts (RENDER-01, RENDER-02, RENDER-04)
...
| 26. UI convergence (impeccable) | 1/3 | In Progress|  |
