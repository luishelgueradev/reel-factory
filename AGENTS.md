...
- **Pipeline worker concurrency**: hard limit **1** (`MAX_CONCURRENT_JOBS` default in `services/api-server/src/constants.ts`). The pipeline is single-job by design — each job spawns a `remotion-renderer` container running headless Chrome, and two in parallel exhaust RAM/shm and OOM mid-render. Never raise this in `docker-compose.yml` or `.env` for production.
